...
public class SpawnerSilkUpdater {
     ...
     */
    public SpawnerSilkUpdater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
     ...
     */
    public SpawnerSilkUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback) {
     ...
     * @param announce True if the program should announce the progress of new updates in console.
     */
    public SpawnerSilkUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
        this.plugin = plugin;
        this.type = type;
        ...
        try {
            this.url = new URL(SpawnerSilkUpdater.HOST + SpawnerSilkUpdater.QUERY + this.id);
...
//            fout = new FileOutputStream(new File(this.updateFolder, file.getName()));
            ...
            final byte[] data = new byte[SpawnerSilkUpdater.BYTE_SIZE];
            int count;
            if (this.announce) {
                this.plugin.getLogger().info("About to download a new update: " + this.versionName);
            }
            long downloaded = 0;
            while ((count = in.read(data, 0, SpawnerSilkUpdater.BYTE_SIZE)) != -1) {
                downloaded += count;
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (!entry.isDirectory()) {
                    ...
                    int b;
                    final byte[] buffer = new byte[SpawnerSilkUpdater.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, SpawnerSilkUpdater.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, SpawnerSilkUpdater.BYTE_SIZE)) != -1) {
    ...
    private boolean hasTag(String version) {
        for (final String string : SpawnerSilkUpdater.NO_UPDATE_TAG) {
    ...
    private boolean read() {
        try {
            ...
            conn.addRequestProperty("User-Agent", SpawnerSilkUpdater.USER_AGENT);
            ...
            this.versionName = (String) latestUpdate.get(SpawnerSilkUpdater.TITLE_VALUE);
            this.versionLink = (String) latestUpdate.get(SpawnerSilkUpdater.LINK_VALUE);
            this.versionType = (String) latestUpdate.get(SpawnerSilkUpdater.TYPE_VALUE);
            this.versionType = (String) latestUpdate.get(SpawnerSilkUpdater.TYPE_VALUE);
            this.versionGameVersion = (String) latestUpdate.get(SpawnerSilkUpdater.VERSION_VALUE);
            this.versionFileName = (String) latestUpdate.get(SpawnerSilkUpdater.VERSION_FILENAME);
    ...
    public interface UpdateCallback {
         ...
         */
        void onFinish(SpawnerSilkUpdater updater);
    }
